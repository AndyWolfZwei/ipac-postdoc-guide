...
def file_content(file_name):
    ...
    if isinstance(file_name, str) and os.path.exists(file_name):
        with open(file_name, 'rU', encoding='UTF-8') as file_object:
...
def content_slice(content):
    ...
    if 'y' == input("--- Need a slice of the file content? ---\n(Y/y for yes; others for no) ").lower():
        start_label = input(
        ...
        end_label = input("------ Paste the last sentence to end with (or leave empty to end at the last) ------\n")
        if end_label != "":
            try:
